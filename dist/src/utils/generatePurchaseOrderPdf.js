...
const adminRoles_1 = require("./adminRoles");
...
const findLogoPath = () => {
    const candidates = [
        path_1.default.join(__dirname, "../assets/po-header-logo.png"),
        path_1.default.join(process.cwd(), "src/assets/po-header-logo.png"),
        path_1.default.join(process.cwd(), "dist/src/assets/po-header-logo.png"),
    ];
    return candidates.find((filePath) => fs_1.default.existsSync(filePath)) || null;
};
...
const pickApprover = (approvals, roles) => {
...
const pickAdminApprover = (approvals) => pickApprover(approvals, [...adminRoles_1.ADMIN_APPROVER_ROLES]);
const catmullRom = (doc, pts) => {
...
const drawMetaRow = (doc, y, leftLabel, leftValue, rightLabel, rightValue) => {
    const leftX = 33;
    const rightX = 307;
    const colWidth = 235;
    doc
...
const drawItemsTable = (doc, data) => {
    ...
    const columns = [
        { header: "#", align: "center" },
        { header: "Item Name", align: "center" },
        { header: "Unit", align: "center" },
        { header: "Per Unit Rate", align: "center" },
        { header: "Item Qty", align: "center" },
    ];
...
const drawSignatureLine = (doc, labelY, roleY, label, role, name) => {
    const labelX = 33;
    doc
        .font("Helvetica-Bold")
        .fontSize(11)
...
const generatePurchaseOrderPdf = (data) => new Promise((resolve, reject) => {
    ...
    doc.on("error", reject);
    drawHeader(doc);
    drawMetaRow(doc, 188, "PO Number", data.referenceNumber, "Creation Date", formatDate(data.createdAt));
    drawMetaRow(doc, 231, "Project", data.projectName, "Section", data.sectionName);
    drawMetaRow(doc, 273, "Vendor Name", data.vendorName, "Deliver To", data.deliverTo);
    ...
    drawSignatureLine(doc, 560, 578, "Created By:", "(Construction Manager)", createdBy);
    drawSignatureLine(doc, 604, 622, "Approved By:", "(Project Manager)", pm);
    ...
    doc
        ...
        .fontSize(9.5)
        .fillColor("#333333")
        .text("System-generated purchase order \u2014 Created and Approved fields are auto-filled by the software.", 60, 778, { width: PAGE_WIDTH - 120, align: "center", lineGap: 2 });
...
//# sourceMappingURL=generatePurchaseOrderPdf.js.map