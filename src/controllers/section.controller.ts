...
const getSections = catchAsync(async (req, res) => {
  ...
  if (user.role === "ADMIN") {
    ...
    if (user.isHead) {
      // Head Store Incharge: sees all sections in their assigned projects
      ...
      // Get all sections in those projects
      ...
      // Also include sections from direct store assignments (if any)
      ...
      // Regular store incharge: only sees sections where they have direct store assignments
    ...
    const hasProjectScope =
      user.isHead || assignments.some((a) => !a.sectionId);
...
const getSectionById = catchAsync(async (req, res, next) => {
  ...
  if (user.role !== "ADMIN") {
    ...
    if (user.role === "SITE_INCHARGE") {
      ...
      if (user.isHead) {
        // Head Store Incharge: check if section belongs to one of their assigned projects
        const section = await prisma.section.findUnique({
          where: { id },
          select: { projectId: true },
        });
      ...
      // Also check direct store assignment (for both head and regular incharge)
      if (!assigned) {
      ...
      if (!assigned) {
        const sectionAssignment = await prisma.accountantAssignment.findFirst({
          where: { userId: user.id, sectionId: id, isActive: true },
        });
